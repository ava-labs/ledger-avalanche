...
static const bagl_element_t view_error[] = {
    UI_FillRectangle(0, 0, 0, UI_SCREEN_WIDTH, UI_SCREEN_HEIGHT, 0x000000,
                     0xFFFFFF),
...
static unsigned int view_error_button(unsigned int button_mask,
...
static unsigned int view_review_button(unsigned int button_mask,
                                       unsigned int button_mask_counter) {
  UNUSED(button_mask_counter);
  switch (button_mask) {
  case BUTTON_EVT_RELEASED | BUTTON_LEFT | BUTTON_RIGHT:
...
void crapoline_ux_display_view_error() { UX_DISPLAY(view_error, view_prepro); }
