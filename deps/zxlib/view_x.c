...
UX_STEP_INIT(ux_review_flow_2_start_step, NULL, NULL,
             { rs_h_review_loop_start(); });
...
UX_STEP_VALID(ux_review_flow_3_step, pb, rs_h_approve(0),
              {&C_icon_validate_14, APPROVE_LABEL});
UX_STEP_VALID(ux_review_flow_4_step, pb, rs_h_reject(0),
              {&C_icon_crossmark, REJECT_LABEL});
...
void crapoline_ux_wait() { UX_WAIT(); }
...
void crapoline_ux_flow_prev() { ux_flow_prev(); }

void crapoline_ux_flow_next() { ux_flow_next(); }

void crapoline_ux_layout_bnnn_paging_reset() { ux_layout_bnnn_paging_reset(); }
