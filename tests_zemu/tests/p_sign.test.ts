...
import { defaultOptions, ED25519_TYPE, models, ROOT_PATH, SECP256K1_TYPE } from './common'
...
describe.each(models)('P_Sign[$name]; sign', function (m) {
  ...
  test.each(LITE_SIGN_TEST_DATA)('sign p-chain $name ed25519', async function ({ name, op }) {
    ...
    try {
      ...
      const signers = ['0/0']
      const respReq = app.sign(ROOT_PATH, signers, msg, signers, ED25519_TYPE)
      ...
      for (const signer of signers) {
        const path = `${ROOT_PATH}/${signer}`
        const resp_addr = await app.getAddressAndPubKey(path, false, undefined, undefined, ED25519_TYPE)
